...
export default function ChatInterface({ 
  ...
  return (
    <motion.div 
      ...
      className="h-screen flex bg-black relative overflow-hidden"
    >
      <div className="mesh-shader opacity-30" />
      ...
      <div className="flex-1 flex flex-col md:flex-row p-2 md:p-4 gap-2 md:gap-4">
        ...
        {/* Chat Stream */}
        <div className="flex-1 flex flex-col bg-black/20 rounded-2xl overflow-hidden order-1 border border-white/5">
           {/* Stream Header */}
           <div className="px-4 py-3 flex items-center justify-between bg-white/[0.02] border-b border-white/5">
              <div className="flex items-center gap-3">
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-neon-cyan/20 to-purple-500/20 border border-white/10 flex items-center justify-center">
                       <User size={18} className="text-white/70" />
                    </div>
                    <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-black ${isTyping ? 'bg-warning-amber' : 'bg-success-emerald'}`} />
                  </div>
                  <div>
              ...
              <div className="flex items-center gap-2">
                 ...
                 <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 rounded-full hover:bg-white/5 transition-colors">
           ...
           >
              ...
              {messages.map((msg, index) => {
                ...
                if (isSystem) return (
                  <div key={msg.id} className="flex justify-center py-2">
                    <span className="text-[10px] font-medium text-white/40 bg-white/5 px-3 py-1.5 rounded-full">
                      {msg.text}
                    </span>
                  </div>
                );
                ...
                return (
                  ...
                  >
                    {!isUser && showAvatar && (
                      <div className="w-8 h-8 shrink-0 rounded-full bg-gradient-to-br from-neon-cyan/30 to-purple-500/30 flex items-center justify-center self-end">
                    ...
                    <div className={`max-w-[75%] flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
                       <div className={`px-4 py-2.5 rounded-2xl ${
                         isUser 
                           ? 'bg-gradient-to-r from-neon-cyan to-cyan-400 text-deep-space rounded-br-md' 
              ...
              {isTyping && (
                ...
                >
                   <div className="w-8 h-8 shrink-0 rounded-full bg-gradient-to-br from-neon-cyan/30 to-purple-500/30 flex items-center justify-center">
                      <User size={12} className="text-white" />
                   </div>
                   <div className="px-4 py-3 bg-white/10 rounded-2xl rounded-bl-md flex gap-1.5">
                      {[0, 1, 2].map(i => (
                        <motion.div 
                          key={i}
           ...
           <div className="p-3 border-t border-white/5">
              <div className="flex gap-2 items-center">
                 ...
                 <input
                   ...
                   placeholder="Message..."
                   className="flex-1 bg-white/5 border border-white/10 rounded-full px-4 py-2.5 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-white/20 transition-all"
                 />
        ...
        <AnimatePresence>
          {sidebarOpen && (
            ...
            >
              <div className="bg-black/30 border border-white/5 rounded-2xl p-5 flex flex-col gap-5">
